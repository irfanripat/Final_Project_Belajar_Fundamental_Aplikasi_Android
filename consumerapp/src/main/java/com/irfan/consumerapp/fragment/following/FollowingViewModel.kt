package com.irfan.consumerapp.fragment.following

import android.util.Log
import androidx.lifecycle.LiveData
import androidx.lifecycle.MutableLiveData
import androidx.lifecycle.ViewModel
import com.irfan.consumerapp.api.ApiInterface
import com.irfan.consumerapp.api.RetrofitClient
...
class FollowingViewModel: ViewModel() {
    ...
    fun getListFollowing(username: String): LiveData<ArrayList<DetailUser>> {
        _isSuccess.value = -1
        val api = RetrofitClient.getRetrofitInstance().create(ApiInterface::class.java)
        val listItem = ArrayList<DetailUser>()
        uiScope.launch {
            try {
                val response = api.getListFollowing(username = username)
